...
def BCCJ_call_value(S0, K, T, r, kappa_v, theta_v, sigma_v, rho, v0,
                   lamb, mu, delta, C):
    ''' Valuation of European call option in B96 Model via Lewis (2001)
    Fourier-based approach.
    ...
    delta: float
        standard deviation of jump
    C: float
        constant jump in volatility squared
    ...
    '''
    int_value = quad(lambda u: BCCJ_int_func(u, S0, K, T, r, kappa_v, theta_v,
                                            sigma_v, rho, v0, lamb, mu, delta, C),
                     0, np.inf, limit=250)[0]
    call_value = max(0, S0 - np.exp(-r * T) * np.sqrt(S0 * K) /
                     np.pi * int_value)
    return call_value
...
def BCCJ_int_func(u, S0, K, T, r, kappa_v, theta_v, sigma_v, rho, v0,
                 lamb, mu, delta, C):
    ''' Valuation of European call option in BCC97 model via Lewis (2001)
    Fourier-based approach: integration function. This has constant jumps in the vol squared

    Parameter definitions see function BCC_call_value.'''
    char_func_value = BCCJ_char_func(u - 1j * 0.5, T, r, kappa_v, theta_v,
                                    sigma_v, rho, v0, lamb, mu, delta, C, 100)
    int_func_value = 1 / (u ** 2 + 0.25) \
        * (np.exp(1j * u * np.log(S0 / K)) * char_func_value).real
    return int_func_value
...
def BCCJ_char_func(u, T, r, kappa_v, theta_v, sigma_v, rho, v0, lamb, mu, delta, C, N):
    ''' Valuation of European call option in BCC model via Lewis (2001)
    Fourier-based approach: characteristic function. This version has a
    constant positive jump C in the variance. N is the number of intervals for
    integration of exp(BC)
    Parameter definitions see function BCC_call_value.'''
    c1 = kappa_v * theta_v
    c2 = -np.sqrt((rho * sigma_v * u * 1j - kappa_v) ** 2 -
                  sigma_v ** 2 * (-u * 1j - u ** 2))
    ...
    H2 = ((kappa_v - rho * sigma_v * u * 1j + c2) / sigma_v ** 2 *
          ((1 - np.exp(c2 * T)) / (1 - c3 * np.exp(c2 * T))))
    ## integral of exp(BC) in the H1 term (A)
    cc = np.ones(N+1)
    cc[0] = 0.5
    cc[N] = 0.5
    cc *= T/N
    ...
    for i in range(N+1):
        tt = i*T/N
        H2J = ((kappa_v - rho * sigma_v * u * 1j + c2) / sigma_v ** 2 *
            ((1 - np.exp(tt*c2)) / (1 - c3 * np.exp(tt*c2))))
        d = np.exp(H2J*C)
        BT += cc[i]*d
    omega = -lamb * (np.exp(mu + 0.5 * delta ** 2) - 1)
    jterm = 1j * u * omega * T + lamb * (np.exp(1j * u * mu - u ** 2 *delta ** 2 * 0.5) - 1) * BT
    H1 += jterm
    char_func_value = np.exp(H1 + H2 * v0)
    return char_func_value
...
if __name__ == '__main__':
    ...
    print("BCC97 Value %10.4f"
          % BCC_call_value(S0, K, T, r, kappa_v, theta_v,
                           sigma_v, rho, v0, lamb, mu, delta)) """
    print("BCCJ Value  %10.4f"
          % BCCJ_call_value(S0, K, T, r, kappa_v, theta_v,
                           sigma_v, rho, v0, lamb, mu, delta, C))
